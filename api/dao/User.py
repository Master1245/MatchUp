...
class UserDao:
    ...
    @classmethod
    def to_dict_api(cls, user: User):
        preference = Preference(name=user.preference) if user.preference else None
        preference_afinity = [Preference(name=pref) for pref in json.loads(user.preference_afinity)] if user.preference_afinity else []
        ...
        return {
            ...
            "bio": user.bio,
            "avatar": user.avatar,
            "list_images": json.loads(user.list_images),
